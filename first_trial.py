...
while True:
    ...
    low_red = np.array([0, 180, 200]) #open hsv app and see the value of the low red color
                                    #HSV = Hue, Saturation, Brightness Value / Hue values is in range(0 - 60) for red
    high_red = np.array([10, 360, 360])

    red_mask = cv2.inRange(hsv_frame, low_red, high_red) # ıt applied the low ang high points and
    # we find what we want in determinant range of red scala.
    ...
    if(matrix.sum() / 255 > 800): #We must detect number of pixel
        ...
        cv2.circle(img=frame, center=(circle_center_point1, circle_center_point2), radius=9, color=(0, 255, 0), thickness=-1)
        ...
        print("Our center point is ({}, {})".format(center_point_x.most_common()[0][0], center_point_y.most_common()[0][0]))
